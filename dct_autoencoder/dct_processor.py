...
class DCTProcessor:
    def __init__(self, channels:int, patch_size:int, dct_compression_factor: float, max_n_patches: int, max_seq_len:int, patch_norm_device='cpu', token_dropout_prob=None, ):
        ...
        self.patch_norm = PatchNorm(patch_size, patch_size, patch_size**2 * channels).to(patch_norm_device)
    ...
    def preprocess(self, x: List[torch.Tensor]) -> DCTPatches:
        ...
        dct_patches.patches = self.patch_norm(dct_patches.patches, dct_patches.h_indices, dct_patches.w_indices)
        return dct_patches
    ...
    def group_images_by_max_seq_len(
        ...
        calc_token_dropout = default(self.calc_token_dropout, always(0.))
        ...
        for image in images:
            ...
            assert image_seq_len <= self.max_seq_len, f'image with dimensions {image_dims} exceeds maximum sequence length'
    ...
    def patch_images(
        ...
        p, c, device, has_token_dropout = self.patch_size, self.channels, torch.device('cpu'), exists(self.calc_token_dropout)
    ...
    def revert_patching(self, output: DCTPatches) -> List[torch.Tensor]:
        ...
        for batch_i, (image_ids, mask, positions) in enumerate(zip(output.batched_image_ids, output.key_pad_mask, output.patch_positions)):
            ...
            for image_id in image_ids.unique():
                ...
                image = rearrange(image_tokens, '(h w) (c p1 p2) -> c (h p1) (w p2)', p1 = self.patch_size, p2=self.patch_size, w=w, h=h)
                images.append(image)
